...
#  avatar                 :string(255)
#  pam_enabled            :boolean          default(FALSE), not null
#  am_enabled             :boolean          default(FALSE), not null
#
...
class Project < ActiveRecord::Base
  ...
  ### project master
  def project_master_member?(user_id)
    project_member = project_members.find_by(user_id: user_id, source_id: id)

    if project_member.nil?
      return false
    end

    if project_member.access_level == Gitlab::Access::MASTER
      return true
    else
      return false
    end
