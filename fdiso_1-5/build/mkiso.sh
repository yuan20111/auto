...
if [ $# -gt 2 ] ; then
    ...
    ISONAME="$iso_name-i386-`date +%Y%m%d%H%M`.iso"
fi
...
xorriso -as mkisofs -quiet -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot \
 -isohybrid-mbr /usr/lib/syslinux/isohdpfx.bin \
 -boot-load-size 4 -boot-info-table -eltorito-alt-boot \
 -e boot/grub/efi.img -no-emul-boot -R -J  -o "$GENISOPATH/$ISONAME" .
