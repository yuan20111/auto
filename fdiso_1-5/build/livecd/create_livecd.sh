...
echo "$OSNAME, $OSVERSION, $OSVERSIONFULLNAME,  $OUTPATH"
isodir="$OSNAME"
...
menu title Welcome to $OSNAME "$OSVERSION" 64-bit
...
echo $OSFULLNAME $OSVERSION "$OSVERSIONFULLNAME" - Release \(`date +%Y%m%d`\) > $OUTPATH/"$isodir"/.disk/info
echo $OSFULLNAME $OSVERSION "$OSVERSIONFULLNAME" - Release \(`date +%Y%m%d`\) > $OUTPATH/"$isodir"/.disk/mint4win
