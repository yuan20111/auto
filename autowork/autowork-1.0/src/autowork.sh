...
NOTEDIR=/home/nfs/compile				#gitlab服务器的note发到这里
if [ $DBG ] ;then
...
message(){
	loglevel=2             #日志输出位置             
	local MESGDATE=`date +[%Y%m%d-%H:%M:%S]`
	case $loglevel in 
		0)
			;;
		1)
...
auto_sendmail2author(){
	...
	cat  $TMPLOG >> /tmp/content.txt
	[ $DBG ] ||	{	
		mail -A $NOTE  $NOTE  -s "dpkg-buildpackage error,please checkout code or debian/changelog format" $MAILIP < $TMPLOG
	}&
	{	
...
copy_deb(){
	...
	cp ../*.deb $DESTDIR/$SRCMARGE
	[ "$?" -ne "0" ] && message "copy error!"
	local flag=0
	local SRCDEB=`ls ../*.deb`
	local DEBNUM=`find $RUNINGDIR/$SRCMARGE -maxdepth 3 -print | grep ".deb$" | grep -v udeb$ | wc -l`
...
build(){
	...
	if [  -n "$DPKGPATH" ];then
		...
		message "build error! cann't find debian/changelog,please manually compile"
		{
			mail -s "$SRCMARGE cann't find debian/changelog,please manually compile" $ADMIN_MAILIP,$ADMIN_MAILIP1 < /dev/null
		}&
		return
	fi  
...
#检查软件分支，确定生成的包拷贝的位置
check_copydir(){
    [ $DBG ] && DESTDIR=~/nfs-deb  || DESTDIR=/home/nfs/autowork/nfs-deb #产出物
    case $branch in  
        master)											#主分支
        ...
        INESA)											#仪电
            ...
            ;;  
        *)  
            message "$DESTDIR is default output dir"		#其他分支包，默认放在nfs-deb
            ;;  
    esac
...
#参数加双引号，包含多个文件或文件夹
bak-note(){
	[ ! -d $2 ] && mkdir -p "$2"  #创建/home/nfs/compile
	cp -r  $1 $2
}
...
monitor(){
	...
	rm -rf $TMPLOG
	AUTOWOTKLOG=/home/nfs/autowork/nfs-deb/autowork.log 
	[ ! -f $AUTOWOTKLOG ] && touch $AUTOWOTKLOG
	while [ True ];do
		#编包最外层入口
		ls $NOTEDIR/*.txt
		if [ $? -eq 0 ];then  #编译note存在
			#备份note
			local BACKUP="/home/nfs/autowork/backup"
			bak-note "$NOTEDIR/*.txt" "$BACKUP"

			local now=`date +%Y/%m/%d-%H:%M:%S`
			[ $DBG ] && echo "$now 发现>>> $(ls $NOTEDIR/*.txt)<<<" >> ~/autowork.log  || echo "[$now] 发现>>> $(ls $NOTEDIR/*.txt)<<<" >>  $AUTOWOTKLOG
			#过滤器,此处数组控制编哪个组的包。
			#方德现状--1.5:"desktop_2015" "nfs-develop",1.0:"bugTeam",2.0:"2.0_cdosDesktop",3.0:"Desktop3.0"
			GRPIPD=("desktop_2015" "nfs-develop" "bugTeam" "2.0_cdosDesktop" "Desktop3.0")
			for i in ${GRPIPD[@]};do
				GRPIP=$i
				echo "-----------------$GRPIP"
				check_group 
			...
			if [ $? -eq 0 ]; then
				...
				#判断32位编译机是否在工作。
				if [ ! -n "`ping 192.168.7.182 -i 0.2 -c 1 | grep error`" ]; then
					[ $DBG ] && echo "将不符要求的note文件，发送到（nfs 192.168.7.182)处理" >> ~/autowork.log  || echo "将不符要求的note文件，发送到（nfs 192.168.7.182)处理" >>  $AUTOWOTKLOG
					scp  $NOTEDIR/*.txt nfs@192.168.7.182:/home/nfs/compile/
					[ $? -eq 0 ] || echo "发送到（nfs 192.168.7.182)成功" >> ~/autowork.log  && echo "发送到（nfs 192.168.7.182)成功" >>  $AUTOWOTKLOG
					[ $? -eq 0 ] && echo "发送到（nfs 192.168.7.182)成功" || echo "发送到（nfs 192.168.7.182)成功" 
				else
					{
						mail -s "32位编译机不通，请您核查一下哦" $ADMIN_MAILIP,$ADMIN_MAILIP1 < /dev/null
					}&
					[ $DBG ] && echo "32位编译机不通请检查" >> ~/autowork.log  || echo "32位编译机不通请检查" >>  $AUTOWOTKLOG
				fi
				#备份发往32的note，即没有自动编包的note
				local BACKUP_NAUTO="/home/nfs/autowork/no-auto-compile"
				bak-note "$NOTEDIR/*.txt" "$BACKUP_NAUTO"
